...
st.set_page_config(page_title="HAMA V6.8 KAMILI", page_icon="🚚", layout="centered")
...
conn = sqlite3.connect('hama_v68.db', check_same_thread=False)
c = conn.cursor()
...
if menu == "Mteja - Book":
    ...
    if pwd=="hama123":
        ...
        st.divider(); st.write("Madereva Pending")
        ...
        for i,j,s,a,kt,pg in c.fetchall():
            with st.container(border=True):
                ...
                if st.button("Kataa "+j, key="kataa_"+str(i)):
                    c.execute("DELETE FROM drivers WHERE id=?",(i,)); conn.commit(); st.rerun()
        st.divider()
        ...
        for oid,mteja,jumla,ref in c.fetchall():
            ...
            if st.button("Thibitisha Oda "+str(oid), key="thibi_"+str(oid)):
                c.execute("UPDATE orders SET status='paid_verified' WHERE id=?",(oid,)); conn.commit(); st.success("Verified")
        st.divider(); st.write("Maoni ya Wateja")
        c.execute("SELECT d.jina,m.stars,m.ujumbe FROM maoni m JOIN drivers d ON m.driver_id=d.id ORDER BY m.id DESC LIMIT 20")
        for j,s,u in c.fetchall(): st.write(j+" | "+"⭐"*s+" "+u)
