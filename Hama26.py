...
if menu == "Mteja - Book":
    ...
    if st.button("Tuma Maombi"):
        if len(nida)<10:
            ...
            conn.commit()
            st.success("Umesajiliwa Kituo "+kituo+" Subiri Admin")

elif menu == "Dereva - Wallet":
    simu_d=st.text_input("Simu yako")
    if simu_d:
        c.execute("SELECT id,jina,kituo FROM drivers WHERE simu=? AND verified=1",(simu_d,))
        dr=c.fetchone()
        if not dr:
    ...
    if pwd=="hama123":
        ...
        ab=c.fetchone()[0]
        st.metric("Commission yako", str(int(ab)))
        st.divider()
        st.write("Madereva Pending - Hawajakubaliwa")
        c.execute("SELECT id,jina,simu,aina,kituo,picha_gari FROM drivers WHERE verified=0")
        pend=c.fetchall()
        for i,j,s,a,kt,pg in pend:
            with st.container(border=True):
                st.write(j+" | "+s+" | "+a+" | "+kt)
